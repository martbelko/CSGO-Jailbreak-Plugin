...
public void OnPluginStart()
{
	...
	RegAdminCmd("sm_sql_addadmin", Command_AddAdmin, ADMFLAG_CUSTOM2, "Adds an admin to the SQL database");
	RegAdminCmd("sm_sql_deladmin", Command_DelAdmin, ADMFLAG_CUSTOM2, "Removes an admin from the SQL database");
	RegAdminCmd("sm_sql_addgroup", Command_AddGroup, ADMFLAG_CUSTOM2, "Adds a group to the SQL database");
	RegAdminCmd("sm_sql_delgroup", Command_DelGroup, ADMFLAG_CUSTOM2, "Removes a group from the SQL database");
	RegAdminCmd("sm_sql_setadmingroups", Command_SetAdminGroups, ADMFLAG_CUSTOM2, "Sets an admin's groups in the SQL database");
...
public Action Command_DelAdmin(int client, int args)
{
	...
	Format(query, 
		sizeof(query),
		"SELECT id, immunity FROM sm_admins WHERE authtype = '%s' AND identity = '%s'",
		authtype,
		safe_identity);
